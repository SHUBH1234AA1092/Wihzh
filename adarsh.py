...
LOG_FILE = "log.txt"
# @MR_ADARSH_YTs. JOIN TO MORE UPDATES 
...
def log_command(user_id, target, port, time):
    ...
    username = "@MR_ADARSH_YTs" + user_info.username if user_info.username else f"UserID: {user_id}"
...
def show_all_users(message):
    ...
    if user_id in admin_id:
        ...
        response = "𝐁𝐎𝐓 𝐅𝐀𝐓𝐇𝐄𝐑 𝐂𝐀𝐍 𝐃𝐎 𝐓𝐇𝐈𝐒 𝐂𝐎??𝐌𝐀𝐍𝐃."
    bot.reply_to(message, response)
...
def start_attack_reply(message, target, port, time):
    ...
    keyboard.add(
        telebot.types.InlineKeyboardButton("SUPPORT", url="https://t.me/+Ldiyy-c0r1Y4YWE1")
    )
...
def show_user_id(message):
    user_id = str(message.chat.id)
    response = f"𝐂A𝐍𝐀𝐑𝐘 𝐀𝐏𝐊 --> @MR_ADARSH_YTs"
    bot.reply_to(message, response, parse_mode='Markdown')

@bot.message_handler(commands=['S4'])
def show_help(message):
    response = """𝐈 𝐊𝐍𝐎𝐖 𝐘𝐎𝐔 𝐂𝐎𝐌𝐌𝐀𝐍𝐃 𝐈𝐒 --> 𝐒𝟒 \n𝐁𝐔𝐓 𝐇𝐈𝐒 𝐁𝐎𝐓𝐒 𝐅𝐀𝐓𝐇𝐄𝐑 @MR_ADARSH_YTs \n BAAP HU TERA MAI MADHARCHOD.
"""
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.row(
        telebot.types.InlineKeyboardButton('Updates', url='https://t.me/+Ldiyy-c0r1Y4YWE1'),
        telebot.types.InlineKeyboardButton('Support', url='https://t.me/+Ldiyy-c0r1Y4YWE1')
    )
